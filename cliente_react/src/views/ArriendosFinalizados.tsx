...
} from "../services/ArriendoFinalizadoService";
import type { ArriendoFinalizadoType } from "../types/arriendo";
import FilaArriendoFinalizado from "../components/FilaArriendoFinalizado";
import { useState } from "react";

export async function loader() {
  const arriendos = await getArriendosFinalizados();
  return arriendos;
}

export default function ArriendosFinalizados() {
  let arriendosInicio = useLoaderData() as ArriendoFinalizadoType[];
  if (!arriendosInicio) {
    arriendosInicio = [];
  }
  ...
  const handleBorrar = async (arriendoId: number) => {
    await borrarArriendo(arriendoId);
    ...
    <div className="container my-5">
      <div className="card card-center shadow p-4">
        <h3 className="card-title text-center text-primary fw-bold mb-4">
          Historial de Arriendos Finalizados
        </h3>
